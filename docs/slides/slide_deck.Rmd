...
- Discuss the difference between the `binwidth` and `bins` arguments to `geom_histogram`
...
4. Stacked proportional plot
  - Preferred if <br> numbers of groups in the second variable $\le 3$
...
library(ggplot2)
ggplot(data = profiles, mapping = aes(x = orientation)) +
  geom_bar(position = "dodge", aes(fill = smokes))
```
